...
<%
	...
	if (beginDateTime == null){
		...
		PreparedStatement insertion = connection.prepareStatement(
				"INSERT INTO Requests (User,Origin,Destination,BeginDateTime,EndDateTime) VALUES (?,?,?,?,?)");
		
		insertion.setString(1, username);
		insertion.setString(2, request.getParameter("origin"));
		insertion.setString(3, request.getParameter("destination"));
		insertion.setTimestamp(4, Timestamp.valueOf(beginDateTime));
		insertion.setTimestamp(5, Timestamp.valueOf(endDateTime));
		...
		if (rowsAffected == 0)
			out.print("Failed to add request.<br><br><form action=\"UserDashboard.jsp\"><input type=\"submit\" value=\"Try Again\"/></form>");
		else
			out.print("Your request has been successfully added.<br><br><form action=\"UserDashboard.jsp\"><input type=\"submit\" value=\"Back to Dashboard\"/></form>");
