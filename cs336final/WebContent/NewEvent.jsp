...
<%
	...
	if (beginDateTime == null){
		...
		PreparedStatement insertion = connection.prepareStatement(
				"INSERT INTO Events (User,EventName,BeginDateTime,EndDateTime) VALUES (?,?,?,?)");
		
		insertion.setString(1, username);
		insertion.setString(2, request.getParameter("eventName"));
		insertion.setTimestamp(3, Timestamp.valueOf(beginDateTime));
		insertion.setTimestamp(4, Timestamp.valueOf(endDateTime));
		...
		if (rowsAffected == 0)
			out.print("Failed to add event.<br><br><form action=\"AddEvent.jsp\"><input type=\"submit\" value=\"Try Again\"/></form>");
		else{
			String decision = request.getParameter("decision");
			
			if(decision.equals("Add Another Event"))
				response.sendRedirect("AddEvent.jsp");
			else if(decision.equals("Add another vehicle"))
